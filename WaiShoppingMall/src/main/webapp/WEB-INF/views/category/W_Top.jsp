...
<script>
	...
	function select()
	{
	   ...
	   if(selectitem == "최신순")
	   {
		   location.href='W_Top?viewmethod=productdate';
	   }
	   else if(selectitem == "가격 높은순")
	   {
		   location.href='W_Top?viewmethod=productpriceup'
	   }
	   else if(selectitem == "가격 낮은순")
	   {
		   location.href='W_Top?viewmethod=productpricedown'
	   }	   
	}
	...
	<div class="all-paging2">
		<c:forEach var="i" begin="1" end="${totalnum2 }" step="1">
           <li>
              ...
              <a href="javascript:void(0);" onclick="location.href='W_Top?page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productdate'}">
              <a href="javascript:void(0);" onclick="location.href='W_Top?viewmethod=productdate&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpriceup'}">
              <a href="javascript:void(0);" onclick="location.href='W_Top?viewmethod=productpriceup&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpricedown'}">
              <a href="javascript:void(0);" onclick="location.href='W_Top?viewmethod=productpricedown&page=${i }'">${i }</a>
            </c:if>
           </li>
		</c:forEach>
		</div>
