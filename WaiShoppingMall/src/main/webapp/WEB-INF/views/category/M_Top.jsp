...
<script>
	...
	function select()
	{
	   ...
	   if(selectitem == "최신순")
	   {
		   location.href='M_Top?viewmethod=productdate';
	   }
	   else if(selectitem == "가격 높은순")
	   {
		   location.href='M_Top?viewmethod=productpriceup'
	   }
	   else if(selectitem == "가격 낮은순")
	   {
		   location.href='M_Top?viewmethod=productpricedown'
	   }	   
	}
	...
	<c:forEach var="imsi" items="${mlist }" varStatus="status">
		<div class = "item">
			...
			<h3><a href="goproductdetail?productcode=${imsi.productcode }">${imsi.productsubject }</a></h3>
               <p id="${status.index }">${imsi.productprice } 원</p>
               <script>
               $(document).ready(function()
               {
               	var pri = $("#${status.index }").text();
               	$("#${status.index }").html(numberWithCommas(pri));
               })
               </script>
		</div>
	</c:forEach>
	...
	<table align="center">
		...
		<c:forEach var="i" begin="1" end="${totalnum2 }" step="1">
			<li>
              ...
              <a href="javascript:void(0);" onclick="location.href='M_Top?page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productdate'}">
              <a href="javascript:void(0);" onclick="location.href='M_Top?viewmethod=productdate&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpriceup'}">
              <a href="javascript:void(0);" onclick="location.href='M_Top?viewmethod=productpriceup&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpricedown'}">
              <a href="javascript:void(0);" onclick="location.href='M_Top?viewmethod=productpricedown&page=${i }'">${i }</a>
            </c:if>
           </li>
		</c:forEach>
		</td>
