...
<script>
	...
	function select()
	{
	   ...
	   if(selectitem == "최신순")
	   {
		   location.href='M_Bottom?viewmethod=productdate';
	   }
	   else if(selectitem == "가격 높은순")
	   {
		   location.href='M_Bottom?viewmethod=productpriceup'
	   }
	   else if(selectitem == "가격 낮은순")
	   {
		   location.href='M_Bottom?viewmethod=productpricedown'
	   }	   
	}
...
<div class="all-paging2">
		<c:forEach var="i" begin="1" end="${totalnum2 }" step="1">
           <li>
              ...
              <a href="javascript:void(0);" onclick="location.href='M_Bottom?page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productdate'}">
              <a href="javascript:void(0);" onclick="location.href='M_Bottom?viewmethod=productdate&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpriceup'}">
              <a href="javascript:void(0);" onclick="location.href='M_Bottom?viewmethod=productpriceup&page=${i }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpricedown'}">
              <a href="javascript:void(0);" onclick="location.href='M_Bottom?viewmethod=productpricedown&page=${i }'">${i }</a>
            </c:if>
           </li>
		</c:forEach>
		</div>
