...
<script>
	...
	function select()
	{
	   ...
	   if(selectitem == "최신순")
	   {
		   location.href='search_product?viewmethod=productdate&searchProduct=${search}';
	   }
	   else if(selectitem == "가격 높은순")
	   {
		   location.href='search_product?viewmethod=productpriceup&searchProduct=${search}';
	   }
	   else if(selectitem == "가격 낮은순")
	   {
		   location.href='search_product?viewmethod=productpricedown&searchProduct=${search}';
	   }	   
	}
...
<div id="all-content">
	<h4>
	<c:if test="${search ==''}">
		총 상품 검색 결과: 총 ${totalnum }건의 상품이 검색되었습니다.
	</c:if>
	<c:if test="${search !='' }">
		"<b style="color:blue;">${search }</b>" 상품 검색 결과: 총 <b style="color:red;">${totalnum }건</b>의 상품이 검색되었습니다.
	</c:if>
	</h4><hr>
	<table>
	<tr>
	<td><select name="viewmethod" id="viewmethod" onchange="select()">
	...
	<c:forEach var="imsi" items="${p_list }">
		<div class = "item">
...
<div class="all-paging2">
		<c:forEach var="i" begin="1" end="${totalnum2 }" step="1">
           <li>
              ...
              <a href="javascript:void(0);" onclick="location.href='search_product?page=${i }&searchProduct=${search }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productdate'}">
              <a href="javascript:void(0);" onclick="location.href='search_product?viewmethod=productdate&page=${i }&searchProduct=${search }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpriceup'}">
              <a href="javascript:void(0);" onclick="location.href='search_product?viewmethod=productpriceup&page=${i }&searchProduct=${search }'">${i }</a>
            </c:if>
            <c:if test="${viewmethod == 'productpricedown'}">
              <a href="javascript:void(0);" onclick="location.href='search_product?viewmethod=productpricedown&page=${i }&searchProduct=${search }'">${i }</a>
            </c:if>
           </li>
		</c:forEach>
		</div>
